#!/bin/bash
# ETH 1980 预警发送脚本（由cron调用）
# 每2分钟运行一次，价格<1980时发送飞书通知
...
" 2>/dev/null)
    ...
    if [ -n "$TOKEN" ]; then
        curl -s -X POST 'https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=open_id' \
            ...
            -H 'Content-Type: application/json' \
            -d "{\"receive_id\":\"ou_ce5a94cfca07b266414b003138b8f1f8\",\"msg_type\":\"text\",\"content\":\"{\\\"text\\\":\\\"🚨 ETH 跌破1980！当前约\\\$$PRICE_F，$NOW。空头信号！建议关注是否企稳，随时准备SHORT机会。\\\"}\"}" > /dev/null 2>&1
